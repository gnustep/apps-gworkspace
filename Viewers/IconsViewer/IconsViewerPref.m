 /*  -*-objc-*-
 *  IconsViewerPref.m: Implementation of the IconsViewerPref Class 
...
static NSString *nibName = @"IconsViewerPref";

@implementation LabelResizer
...
@implementation IconsViewerPref
...
- (id)init
{
  ...
  if (self) {    
      ...
      widthStr = [defaults objectForKey: @"iconsCellsWidth"];

      if (widthStr) {
        cellsWidth = [widthStr intValue];
      } else {
        cellsWidth = DEFAULT_ICONS_WIDTH;
      }

      orx = (int)((BOX_W - cellsWidth) / 2);

      leftResizer = [[LabelResizer alloc] initForController: self 
      ...
      rightResizer = [[LabelResizer alloc] initForController: self 
...
- (void)startMouseEvent:(NSEvent *)event onResizer:(LabelResizer *)resizer
{
  NSApplication	*app = [NSApplication sharedApplication];
  NSDate *farAway = [NSDate distantFuture];
  ResizerPosition pos = [resizer position];
