            ...
            remoteHost:(NSString *)rhost
{
  self = [super init];
    ...
    setPaths = [[Column class] instanceMethodForSelector: setPathsSel];
...
- (void)setPathAndSelection:(NSArray *)selection
{
  ...
  for (i = 0; i < numberOfSubStrings; i++) {
		Column *bc = (*getImp)(columns, getSel, column + i);
  ...
  if ([selection count] > 1) {
		Column *bc = (*getImp)(columns, getSel, lastColumnLoaded);
...
- (void)reloadFromColumnWithPath:(NSString *)cpath
{
  ...
  if (col) {
    ...
    int i = 0;
      ...
      Column *nextcol = (*getImp)(columns, getSel, i);
    ...
    if (col) {
      ...
      if (selection) {
        Column *nextcol = (*getImp)(columns, getSel, ([col index] + 1));
...
- (void)tile
{
  ...
  for (i = 0; i < [columns count]; i++) {
		Column *bc = (*getImp)(columns, getSel, i);
...
- (NSString *)pathToLastColumn
{
  ...
  for (i = 0; i < [columns count]; i++) {
    Column *col = (*getImp)(columns, getSel, i);
    ...
    if ([col isLeaf]) {
      ...
      if (is_dir) {	
        if (([gwremote server: remoteHostName isPakageAtPath: cpath] == NO)) {
          return cpath;                  
        } else if (i > 0) {
...
- (NSPoint)positionForSlidedImage
{
  if ((lastVisibleColumn < [columns count])) {
    Column *col = (*getImp)(columns, getSel, lastVisibleColumn);
    NSRect r = [[col iconView] frame];
...
- (void)controlTextDidEndEditing:(NSNotification *)aNotification
{
  ...
  [gwremote server: remoteHostName 
        renamePath: oldpath  
         toNewName: newpath];
}
