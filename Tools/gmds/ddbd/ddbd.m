...
{
  if ([sqlite executeQuery: @"BEGIN"] == NO) {      
    NSLog(@"error at insertPath: %@", path);  
    return;   
  }     
  if (insertPathIfNeeded(path, sqlite) != -1) {      
...
{  
  ...
  if ([sqlite executeQuery: @"COMMIT"] == NO) {      
    NSLog(@"error at: %@", path); 
  }   
                     ...
                     atPath:(NSString *)path
{
  if ([fm fileExistsAtPath: path]) {
    ...
    return [sqlite getBlobEntryWithStatement: statement];
  }
              ...
              atPath:(NSString *)path
{
  if ([fm fileExistsAtPath: path]) {
    ...
    path_id = insertPathIfNeeded(path, sqlite);
...
- (NSTimeInterval)timestampOfPath:(NSString *)path
{
  if ([fm fileExistsAtPath: path]) {
    ...
    stamp = [sqlite getFloatEntryWithStatement: statement];
    
    return ((stamp != FLT_MAX) ? stamp : 0.0);
  }
...
- (BOOL)opendb
{
  ...
  if ([sqlite opendbAtPath: dbpath isNew: &newdb]) {
    if (newdb) {
      if ([sqlite executeSimpleQuery: user_db_schema] == NO) {
    ...
    if (type == NSFileTypeDirectory) {
      ...
      while ((part = [enumerator nextObject]) != nil) {
        ...
        if ([[enumerator fileAttributes] fileType] == NSFileTypeDirectory) {
          if (insertPathIfNeeded(subpath, sqlite) == -1) {      
            [sqlite executeQuery: @"COMMIT"]; 
...
- (oneway void)removeTrees:(NSData *)info
{
  ...
  for (i = 0; i < [paths count]; i++) {
    if (removePath([paths objectAtIndex: i], sqlite) == NO) {
      NSLog(@"error at removeTrees"); 
      GWDebugLog(@"db update failed");
      return;   
    }
...
int insertPathIfNeeded(NSString *path, SQLite *sqlite)
{
...
BOOL removePath(NSString *path, SQLite *sqlite)
{
  ...
  statement = [sqlite statementForQuery: @"DELETE FROM user_paths_removed_id" 
...
BOOL renamePath(NSString *path, NSString *oldpath, SQLite *sqlite)
{
  ...
  statement = [sqlite statementForQuery: @"DELETE FROM user_renamed_paths_base" 
...
BOOL copyPath(NSString *srcpath, NSString *dstpath, SQLite *sqlite)
{
  ...
  statement = [sqlite statementForQuery: @"DELETE FROM user_copied_paths_base" 
